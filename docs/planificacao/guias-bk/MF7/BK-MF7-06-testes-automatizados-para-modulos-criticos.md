...
- [ ] Sei apontar o requisito `RNF28` e demostrar cobertura objetiva.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF7-06: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bkId: 'BK-MF7-06', payload };
}
```
