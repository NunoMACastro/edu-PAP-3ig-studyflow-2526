# BK-MF7-05 - Auditoria completa (materiais, IA, papéis).
...
Neste BK vamos entregar a capacidade correspondente a `RF56`: **Auditoria completa (materiais, IA, papéis).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF56` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-06` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-05`.

## O que entra (scope)
- Definir entrega mínima validável para `RF56` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-06`.
...
- Quando o requisito `RF56` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-05` e dependências `BK-MF7-04`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF56` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Auditoria completa (materiais, IA, papéis).`.
...
- [ ] Resultado observado confirma a entrega de `RF56`.
...
## Critérios de aceite
- O entregável principal de `RF56` está implementado e verificável.
