# BK-MF7-12 - Proteções contra XSS, CSRF, Injection, brute force.

## Header
- `doc_id`: `GUIA-BK-MF7-12`
- `bk_id`: `BK-MF7-12`
- `macro`: `MF7`
- `owner`: `Natália`
- `apoio`: `Guilherme`
...
Neste BK vamos entregar a capacidade correspondente a `RNF17`: **Proteções contra XSS, CSRF, Injection, brute force.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF7` (Privacidade/admin/operação) e na `Fase 3`. Dependências atuais: -; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RNF17` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-13` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-12`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF17` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-13`.
...
- Quando o requisito `RNF17` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-12` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF17` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Proteções contra XSS, CSRF, Injection, brute force.`.
...
- [ ] Resultado observado confirma a entrega de `RNF17`.
...
## Critérios de aceite
- O entregável principal de `RNF17` está implementado e verificável.
