# BK-MF7-09 - HTTPS obrigatório (TLS 1.2+).
...
Neste BK vamos entregar a capacidade correspondente a `RNF14`: **HTTPS obrigatório (TLS 1.2+).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF14` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-10` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-09`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF14` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-10`.
...
- Quando o requisito `RNF14` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-09` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF14` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `HTTPS obrigatório (TLS 1.2+).`.
...
- [ ] Resultado observado confirma a entrega de `RNF14`.
...
## Critérios de aceite
- O entregável principal de `RNF14` está implementado e verificável.
