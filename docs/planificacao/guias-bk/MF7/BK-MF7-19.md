# BK-MF7-19 - Endpoint de health-check.

## Header
- `doc_id`: `GUIA-BK-MF7-19`
- `bk_id`: `BK-MF7-19`
- `macro`: `MF7`
- `owner`: `Guilherme`
- `apoio`: `Natália`
...
Neste BK vamos entregar a capacidade correspondente a `RNF30`: **Endpoint de health-check.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF30` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF8-01` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-19`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF30` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF8-01`.
...
- Quando o requisito `RNF30` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-19` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF30` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Endpoint de health-check.`.
...
- [ ] Resultado observado confirma a entrega de `RNF30`.
...
## Critérios de aceite
- O entregável principal de `RNF30` está implementado e verificável.
