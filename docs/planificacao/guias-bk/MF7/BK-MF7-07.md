# BK-MF7-07 - Definir **quotas de IA** por aluno/turma/grupo e monitorizar consumo.
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF7-06`
- `rf_rnf`: `RF58`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar a capacidade correspondente a `RF58`: **Definir **quotas de IA** por aluno/turma/grupo e monitorizar consumo.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF7` (Privacidade/admin/operação) e na `Fase 3`. Dependências atuais: BK-MF7-06; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF58` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-08` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-07`.

## O que entra (scope)
- Definir entrega mínima validável para `RF58` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-08`.
...
- Quando o requisito `RF58` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-07` e dependências `BK-MF7-06`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF58` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Definir **quotas de IA** por aluno/turma/grupo e monitorizar consumo.`.
...
- [ ] Resultado observado confirma a entrega de `RF58`.
...
## Critérios de aceite
- O entregável principal de `RF58` está implementado e verificável.
