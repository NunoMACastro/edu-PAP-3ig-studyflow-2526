# BK-MF7-10 - Passwords com hashing seguro (bcrypt/argon2).
...
Neste BK vamos entregar a capacidade correspondente a `RNF15`: **Passwords com hashing seguro (bcrypt/argon2).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF15` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-11` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-10`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF15` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-11`.
...
- Quando o requisito `RNF15` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-10` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF15` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Passwords com hashing seguro (bcrypt/argon2).`.
...
- [ ] Resultado observado confirma a entrega de `RNF15`.
...
## Critérios de aceite
- O entregável principal de `RNF15` está implementado e verificável.
