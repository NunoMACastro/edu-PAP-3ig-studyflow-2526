# BK-MF7-13 - Backups diários automáticos.

## Header
- `doc_id`: `GUIA-BK-MF7-13`
- `bk_id`: `BK-MF7-13`
- `macro`: `MF7`
- `owner`: `Guilherme`
- `apoio`: `Natália`
...
Neste BK vamos entregar a capacidade correspondente a `RNF21`: **Backups diários automáticos.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF21` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-14` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-13`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF21` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-14`.
...
- Quando o requisito `RNF21` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-13` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF21` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Backups diários automáticos.`.
...
- [ ] Resultado observado confirma a entrega de `RNF21`.
...
## Critérios de aceite
- O entregável principal de `RNF21` está implementado e verificável.
