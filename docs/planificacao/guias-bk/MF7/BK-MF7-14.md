# BK-MF7-14 - Auto-recovery após falhas.

## Header
- `doc_id`: `GUIA-BK-MF7-14`
- `bk_id`: `BK-MF7-14`
- `macro`: `MF7`
- `owner`: `Guilherme`
- `apoio`: `Natália`
...
Neste BK vamos entregar a capacidade correspondente a `RNF22`: **Auto-recovery após falhas.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF22` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-15` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-14`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF22` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-15`.
...
- Quando o requisito `RNF22` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-14` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF22` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Auto-recovery após falhas.`.
...
- [ ] Resultado observado confirma a entrega de `RNF22`.
...
## Critérios de aceite
- O entregável principal de `RNF22` está implementado e verificável.
