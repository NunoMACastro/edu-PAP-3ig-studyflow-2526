# BK-MF7-16 - Downtime máximo aceitável < 1h/mês.

## Header
- `doc_id`: `GUIA-BK-MF7-16`
- `bk_id`: `BK-MF7-16`
- `macro`: `MF7`
- `owner`: `Kaua`
- `apoio`: `Guilherme`
...
Neste BK vamos entregar a capacidade correspondente a `RNF24`: **Downtime máximo aceitável < 1h/mês.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF24` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-17` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-16`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF24` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-17`.
...
- Quando o requisito `RNF24` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-16` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF24` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Downtime máximo aceitável < 1h/mês.`.
...
- [ ] Resultado observado confirma a entrega de `RNF24`.
...
## Critérios de aceite
- O entregável principal de `RNF24` está implementado e verificável.
