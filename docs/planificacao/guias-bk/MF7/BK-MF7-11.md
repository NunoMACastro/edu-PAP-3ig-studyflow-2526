# BK-MF7-11 - Sessões com cookies HttpOnly + Secure + SameSite.
...
Neste BK vamos entregar a capacidade correspondente a `RNF16`: **Sessões com cookies HttpOnly + Secure + SameSite.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF16` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-12` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-11`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF16` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-12`.
...
- Quando o requisito `RNF16` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-11` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF16` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Sessões com cookies HttpOnly + Secure + SameSite.`.
...
- [ ] Resultado observado confirma a entrega de `RNF16`.
...
## Critérios de aceite
- O entregável principal de `RNF16` está implementado e verificável.
