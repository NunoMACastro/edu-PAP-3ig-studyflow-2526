# BK-MF7-04 - Gestão de utilizadores e papéis.
...
Neste BK vamos entregar a capacidade correspondente a `RF55`: **Gestão de utilizadores e papéis.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF7` (Privacidade/admin/operação) e na `Fase 3`. Dependências atuais: BK-MF0-04; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF55` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-05` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-04`.

## O que entra (scope)
- Definir entrega mínima validável para `RF55` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-05`.
...
- Quando o requisito `RF55` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-04` e dependências `BK-MF0-04`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF55` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Gestão de utilizadores e papéis.`.
...
- [ ] Resultado observado confirma a entrega de `RF55`.
...
## Critérios de aceite
- O entregável principal de `RF55` está implementado e verificável.
