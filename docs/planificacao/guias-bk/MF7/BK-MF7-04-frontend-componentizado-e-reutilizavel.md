...
- [ ] Sei apontar o requisito `RNF26` e demostrar cobertura objetiva.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF7-04: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bkId: 'BK-MF7-04', payload };
}
```
