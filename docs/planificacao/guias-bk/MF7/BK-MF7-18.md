# BK-MF7-18 - Deploy com rollback.

## Header
- `doc_id`: `GUIA-BK-MF7-18`
- `bk_id`: `BK-MF7-18`
- `macro`: `MF7`
- `owner`: `Guilherme`
- `apoio`: `Natália`
...
Neste BK vamos entregar a capacidade correspondente a `RNF29`: **Deploy com rollback.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF29` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-19` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-18`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF29` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-19`.
...
- Quando o requisito `RNF29` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-18` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF29` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Deploy com rollback.`.
...
- [ ] Resultado observado confirma a entrega de `RNF29`.
...
## Critérios de aceite
- O entregável principal de `RNF29` está implementado e verificável.
