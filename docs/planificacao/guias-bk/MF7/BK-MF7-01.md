# BK-MF7-01 - Exportar dados pessoais.
...
Neste BK vamos entregar a capacidade correspondente a `RF52`: **Exportar dados pessoais.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF52` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF7` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-02` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF7-01`.

## O que entra (scope)
- Definir entrega mínima validável para `RF52` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-02`.
...
- Quando o requisito `RF52` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF7-01` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF52` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Exportar dados pessoais.`.
...
- [ ] Resultado observado confirma a entrega de `RF52`.
...
## Critérios de aceite
- O entregável principal de `RF52` está implementado e verificável.
