...
## Contexto do BK
- Entrega alvo: `Submeter materiais da disciplina (versão oficial).` com rastreabilidade direta para `RF21`.
...
- [ ] Sei explicar como `RF21` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF1-09` e do requisito `RF21`.
...
export function autorizarContexto(c: Contexto) {
  ...
  return { bkId: 'BK-MF1-09', req: 'RF21', autorizado: true };
}
```

Evita operações docentes fora do contexto da turma/disciplina.
- Requisitos alvo deste BK: `RF21`.
