# BK-MF1-01 - Criar “Áreas de Estudo” (auto-disciplina independente).
...
Neste BK vamos entregar a capacidade correspondente a `RF07`: **Criar “Áreas de Estudo” (auto-disciplina independente).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF1` (Estudo individual e IA privada) e na `Fase 1`. Dependências atuais: BK-MF0-03; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF07` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF1` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF1-02` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF1-01`.

## O que entra (scope)
- Definir entrega mínima validável para `RF07` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF1-02`.
...
- Quando o requisito `RF07` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF1-01` e dependências `BK-MF0-03`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF07` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Criar “Áreas de Estudo” (auto-disciplina independente).`.
...
- [ ] Resultado observado confirma a entrega de `RF07`.
...
## Critérios de aceite
- O entregável principal de `RF07` está implementado e verificável.
