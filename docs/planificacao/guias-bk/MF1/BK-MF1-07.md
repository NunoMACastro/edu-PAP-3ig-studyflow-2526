# BK-MF1-07 - A IA deve adaptar explicações ao ritmo/dificuldades do aluno.
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF1-05`
- `rf_rnf`: `RF13`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar a capacidade correspondente a `RF13`: **A IA deve adaptar explicações ao ritmo/dificuldades do aluno.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF1` (Estudo individual e IA privada) e na `Fase 1`. Dependências atuais: BK-MF1-05; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF13` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF1` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF1-08` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF1-07`.

## O que entra (scope)
- Definir entrega mínima validável para `RF13` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF1-08`.
...
- Quando o requisito `RF13` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF1-07` e dependências `BK-MF1-05`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF13` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `A IA deve adaptar explicações ao ritmo/dificuldades do aluno.`.
...
- [ ] Resultado observado confirma a entrega de `RF13`.
...
## Critérios de aceite
- O entregável principal de `RF13` está implementado e verificável.
