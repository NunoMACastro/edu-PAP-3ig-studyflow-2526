# BK-MF1-06 - Obter explicações, cards e quizzes personalizados.
...
Neste BK vamos entregar a capacidade correspondente a `RF12`: **Obter explicações, cards e quizzes personalizados.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF12` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF1` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF1-07` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF1-06`.

## O que entra (scope)
- Definir entrega mínima validável para `RF12` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF1-07`.
...
- Quando o requisito `RF12` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF1-06` e dependências `BK-MF1-05`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF12` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Obter explicações, cards e quizzes personalizados.`.
...
- [ ] Resultado observado confirma a entrega de `RF12`.
...
## Critérios de aceite
- O entregável principal de `RF12` está implementado e verificável.
