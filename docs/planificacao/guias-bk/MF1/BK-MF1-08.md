# BK-MF1-08 - IA adapta explicações ao nível do aluno.
...
Neste BK vamos entregar a capacidade correspondente a `RNF36`: **IA adapta explicações ao nível do aluno.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF1` (Estudo individual e IA privada) e na `Fase 1`. Dependências atuais: -; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RNF36` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF1` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF2-01` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF1-08`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF36` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF2-01`.
...
- Quando o requisito `RNF36` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF1-08` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF36` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `IA adapta explicações ao nível do aluno.`.
...
- [ ] Resultado observado confirma a entrega de `RNF36`.
...
## Critérios de aceite
- O entregável principal de `RNF36` está implementado e verificável.
