...
## Contexto do BK
- Entrega alvo: `Partilhar materiais e apontamentos na sala.` com rastreabilidade direta para `RF15`.
...
- [ ] Sei explicar como `RF15` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF1-03` e do requisito `RF15`.
...
export async function registarAluno(input: Credenciais) {
  ...
  return { bkId: 'BK-MF1-03', req: 'RF15', estado: 'REGISTADO' };
}
```

Garante validação mínima de identidade no arranque do fluxo de conta.
- Requisitos alvo deste BK: `RF15`.
