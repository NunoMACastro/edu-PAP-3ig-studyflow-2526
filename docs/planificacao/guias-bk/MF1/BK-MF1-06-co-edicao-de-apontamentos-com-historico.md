# BK-MF1-06 - Co-edição de apontamentos com histórico.
...
- `guia_path`: `docs/planificacao/guias-bk/MF1/BK-MF1-06-co-edicao-de-apontamentos-com-historico.md`
- `last_updated`: `2026-04-14`

## Contexto do BK
- Entrega alvo: `Co-edição de apontamentos com histórico.` com rastreabilidade direta para `RF18`.
...
- [ ] Sei apontar o requisito `RF18` e demostrar cobertura objetiva.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF1-06` e do requisito `RF18`.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF1-06: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bkId: 'BK-MF1-06', payload };
}
```
