...
- [ ] Sei explicar como `RF24` se traduz em comportamento implementável.
...
export function podeNotificar(pref: Preferencia, enviadosHoje: number, quota: number) {
  if (!pref.ativo) return { bkId: 'BK-MF1-12', req: 'RF24', enviar: false, motivo: 'opt-out' };
  return { bkId: 'BK-MF1-12', req: 'RF24', enviar: enviadosHoje < quota };
}
```

Impõe preferências e quota máxima antes do envio.
- Requisitos alvo deste BK: `RF24`.
