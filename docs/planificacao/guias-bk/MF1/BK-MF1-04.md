# BK-MF1-04 - Criar perfil IA da Área de Estudo.
...
Neste BK vamos entregar a capacidade correspondente a `RF10`: **Criar perfil IA da Área de Estudo.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF1` (Estudo individual e IA privada) e na `Fase 1`. Dependências atuais: BK-MF1-02; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF10` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF1` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF1-05` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF1-04`.

## O que entra (scope)
- Definir entrega mínima validável para `RF10` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF1-05`.
...
- Quando o requisito `RF10` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF1-04` e dependências `BK-MF1-02`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF10` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Criar perfil IA da Área de Estudo.`.
...
- [ ] Resultado observado confirma a entrega de `RF10`.
...
## Critérios de aceite
- O entregável principal de `RF10` está implementado e verificável.
