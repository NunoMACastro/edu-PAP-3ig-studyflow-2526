# BK-MF1-05 - Obter resumos IA baseados nos materiais enviados.
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF1-02, BK-MF1-04`
- `rf_rnf`: `RF11`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar a capacidade correspondente a `RF11`: **Obter resumos IA baseados nos materiais enviados.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF1` (Estudo individual e IA privada) e na `Fase 1`. Dependências atuais: BK-MF1-02, BK-MF1-04; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF11` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF1` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF1-06` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF1-05`.

## O que entra (scope)
- Definir entrega mínima validável para `RF11` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF1-06`.
...
- Quando o requisito `RF11` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF1-05` e dependências `BK-MF1-02, BK-MF1-04`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF11` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Obter resumos IA baseados nos materiais enviados.`.
...
- [ ] Resultado observado confirma a entrega de `RF11`.
...
## Critérios de aceite
- O entregável principal de `RF11` está implementado e verificável.
