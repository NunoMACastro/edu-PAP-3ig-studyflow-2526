# BK-MF5-02 - Single Sign-On escolar (OAuth/SAML).
...
- `guia_path`: `docs/planificacao/guias-bk/MF5/BK-MF5-02-single-sign-on-escolar-oauth-saml.md`
- `last_updated`: `2026-04-14`

## Contexto do BK
- Entrega alvo: `Single Sign-On escolar (OAuth/SAML).` com rastreabilidade direta para `RF62`.
...
- [ ] Sei apontar o requisito `RF62` e demostrar cobertura objetiva.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF5-02` e do requisito `RF62`.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF5-02: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bkId: 'BK-MF5-02', payload };
}
```
