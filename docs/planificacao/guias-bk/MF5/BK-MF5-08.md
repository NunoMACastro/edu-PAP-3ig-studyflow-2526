# BK-MF5-08 - Agendar sessões de estudo coletivo.
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF5-06`
- `rf_rnf`: `RF43`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar a capacidade correspondente a `RF43`: **Agendar sessões de estudo coletivo.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF5` (Colaboração e co-estudo) e na `Fase 2`. Dependências atuais: BK-MF5-06; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF43` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF5` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF5-09` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF5-08`.

## O que entra (scope)
- Definir entrega mínima validável para `RF43` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF5-09`.
...
- Quando o requisito `RF43` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF5-08` e dependências `BK-MF5-06`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF43` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Agendar sessões de estudo coletivo.`.
...
- [ ] Resultado observado confirma a entrega de `RF43`.
...
## Critérios de aceite
- O entregável principal de `RF43` está implementado e verificável.
