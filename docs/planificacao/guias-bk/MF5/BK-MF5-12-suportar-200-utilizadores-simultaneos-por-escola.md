...
- [ ] Sei apontar o requisito `RNF10` e demostrar cobertura objetiva.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF5-12: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bkId: 'BK-MF5-12', payload };
}
```
