# BK-MF5-03 - IA partilhada da sala (mistura das áreas dos membros).
...
Neste BK vamos entregar a capacidade correspondente a `RF16`: **IA partilhada da sala (mistura das áreas dos membros).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF5` (Colaboração e co-estudo) e na `Fase 2`. Dependências atuais: BK-MF5-01; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF16` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF5` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF5-04` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF5-03`.

## O que entra (scope)
- Definir entrega mínima validável para `RF16` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF5-04`.
...
- Quando o requisito `RF16` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF5-03` e dependências `BK-MF5-01`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF16` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `IA partilhada da sala (mistura das áreas dos membros).`.
...
- [ ] Resultado observado confirma a entrega de `RF16`.
...
## Critérios de aceite
- O entregável principal de `RF16` está implementado e verificável.
