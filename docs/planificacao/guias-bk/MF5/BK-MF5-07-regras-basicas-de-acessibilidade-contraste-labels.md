...
- [ ] Sei apontar o requisito `RNF05` e demostrar cobertura objetiva.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF5-07: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bkId: 'BK-MF5-07', payload };
}
```
