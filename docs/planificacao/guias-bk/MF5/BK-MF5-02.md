# BK-MF5-02 - Partilhar materiais e apontamentos na sala.
...
Neste BK vamos entregar a capacidade correspondente a `RF15`: **Partilhar materiais e apontamentos na sala.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF15` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF5` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF5-03` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF5-02`.

## O que entra (scope)
- Definir entrega mínima validável para `RF15` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF5-03`.
...
- Quando o requisito `RF15` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF5-02` e dependências `BK-MF5-01`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF15` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Partilhar materiais e apontamentos na sala.`.
...
- [ ] Resultado observado confirma a entrega de `RF15`.
...
## Critérios de aceite
- O entregável principal de `RF15` está implementado e verificável.
