# BK-MF5-06 - Criar grupos de estudo.
...
Neste BK vamos entregar a capacidade correspondente a `RF41`: **Criar grupos de estudo.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF41` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF5` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF5-07` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF5-06`.

## O que entra (scope)
- Definir entrega mínima validável para `RF41` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF5-07`.
...
- Quando o requisito `RF41` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF5-06` e dependências `BK-MF5-01`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF41` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Criar grupos de estudo.`.
...
- [ ] Resultado observado confirma a entrega de `RF41`.
...
## Critérios de aceite
- O entregável principal de `RF41` está implementado e verificável.
