...
- [ ] Sei apontar o requisito `RNF04` e demostrar cobertura objetiva.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF5-06: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bkId: 'BK-MF5-06', payload };
}
```
