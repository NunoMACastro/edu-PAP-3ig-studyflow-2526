# BK-MF5-01 - Criar salas de estudo com outros alunos (livres ou por disciplina).
...
Neste BK vamos entregar a capacidade correspondente a `RF14`: **Criar salas de estudo com outros alunos (livres ou por disciplina).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF5` (Colaboração e co-estudo) e na `Fase 2`. Dependências atuais: BK-MF0-03; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF14` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF5` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF5-02` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF5-01`.

## O que entra (scope)
- Definir entrega mínima validável para `RF14` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF5-02`.
...
- Quando o requisito `RF14` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF5-01` e dependências `BK-MF0-03`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF14` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Criar salas de estudo com outros alunos (livres ou por disciplina).`.
...
- [ ] Resultado observado confirma a entrega de `RF14`.
...
## Critérios de aceite
- O entregável principal de `RF14` está implementado e verificável.
