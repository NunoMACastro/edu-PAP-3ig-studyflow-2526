# BK-MF6-01 - Pesquisa unificada por tópico/conceito/material.
...
Neste BK vamos entregar a capacidade correspondente a `RF45`: **Pesquisa unificada por tópico/conceito/material.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF6` (Pesquisa e notificações) e na `Fase 3`. Dependências atuais: BK-MF2-01; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF45` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF6` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF6-02` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF6-01`.

## O que entra (scope)
- Definir entrega mínima validável para `RF45` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF6-02`.
...
- Quando o requisito `RF45` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF6-01` e dependências `BK-MF2-01`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF45` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Pesquisa unificada por tópico/conceito/material.`.
...
- [ ] Resultado observado confirma a entrega de `RF45`.
...
## Critérios de aceite
- O entregável principal de `RF45` está implementado e verificável.
