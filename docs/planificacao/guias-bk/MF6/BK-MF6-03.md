# BK-MF6-03 - Configurar preferências de notificações (email, push, app) por contexto.
...
Neste BK vamos entregar a capacidade correspondente a `RF47`: **Configurar preferências de notificações (email, push, app) por contexto.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF6` (Pesquisa e notificações) e na `Fase 3`. Dependências atuais: BK-MF0-02; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF47` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF6` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF6-04` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF6-03`.

## O que entra (scope)
- Definir entrega mínima validável para `RF47` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF6-04`.
...
- Quando o requisito `RF47` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF6-03` e dependências `BK-MF0-02`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF47` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Configurar preferências de notificações (email, push, app) por contexto.`.
...
- [ ] Resultado observado confirma a entrega de `RF47`.
...
## Critérios de aceite
- O entregável principal de `RF47` está implementado e verificável.
