# BK-MF6-04 - Alertar alunos sobre rotinas, objetivos e sessões de estudo agendadas.
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF0-05`
- `rf_rnf`: `RF48`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar a capacidade correspondente a `RF48`: **Alertar alunos sobre rotinas, objetivos e sessões de estudo agendadas.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF6` (Pesquisa e notificações) e na `Fase 3`. Dependências atuais: BK-MF0-05; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF48` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF6` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF6-05` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF6-04`.

## O que entra (scope)
- Definir entrega mínima validável para `RF48` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF6-05`.
...
- Quando o requisito `RF48` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF6-04` e dependências `BK-MF0-05`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF48` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Alertar alunos sobre rotinas, objetivos e sessões de estudo agendadas.`.
...
- [ ] Resultado observado confirma a entrega de `RF48`.
...
## Critérios de aceite
- O entregável principal de `RF48` está implementado e verificável.
