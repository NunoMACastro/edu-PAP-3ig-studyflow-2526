# BK-MF6-05 - Notificar grupos/turmas sobre novos materiais, feedback e tarefas.
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF3-06`
- `rf_rnf`: `RF49`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar a capacidade correspondente a `RF49`: **Notificar grupos/turmas sobre novos materiais, feedback e tarefas.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF6` (Pesquisa e notificações) e na `Fase 3`. Dependências atuais: BK-MF3-06; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF49` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF6` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF6-06` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF6-05`.

## O que entra (scope)
- Definir entrega mínima validável para `RF49` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF6-06`.
...
- Quando o requisito `RF49` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF6-05` e dependências `BK-MF3-06`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF49` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Notificar grupos/turmas sobre novos materiais, feedback e tarefas.`.
...
- [ ] Resultado observado confirma a entrega de `RF49`.
...
## Critérios de aceite
- O entregável principal de `RF49` está implementado e verificável.
