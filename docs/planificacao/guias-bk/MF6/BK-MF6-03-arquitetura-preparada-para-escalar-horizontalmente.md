...
- [ ] Sei apontar o requisito `RNF13` e demostrar cobertura objetiva.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF6-03: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bkId: 'BK-MF6-03', payload };
}
```
