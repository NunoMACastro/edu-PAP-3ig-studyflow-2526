# BK-MF6-07 - Administradores configuram canais e quotas máximas de notificações.
...
Neste BK vamos entregar a capacidade correspondente a `RF51`: **Administradores configuram canais e quotas máximas de notificações.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF6` (Pesquisa e notificações) e na `Fase 3`. Dependências atuais: BK-MF6-06; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF51` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF6` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF7-01` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF6-07`.

## O que entra (scope)
- Definir entrega mínima validável para `RF51` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF7-01`.
...
- Quando o requisito `RF51` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF6-07` e dependências `BK-MF6-06`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF51` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Administradores configuram canais e quotas máximas de notificações.`.
...
- [ ] Resultado observado confirma a entrega de `RF51`.
...
## Critérios de aceite
- O entregável principal de `RF51` está implementado e verificável.
