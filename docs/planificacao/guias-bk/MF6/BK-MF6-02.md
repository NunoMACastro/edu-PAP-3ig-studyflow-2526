# BK-MF6-02 - Navegação por programa/currículo.
...
Neste BK vamos entregar a capacidade correspondente a `RF46`: **Navegação por programa/currículo.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF46` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF6` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF6-03` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF6-02`.

## O que entra (scope)
- Definir entrega mínima validável para `RF46` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF6-03`.
...
- Quando o requisito `RF46` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF6-02` e dependências `BK-MF2-01`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF46` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Navegação por programa/currículo.`.
...
- [ ] Resultado observado confirma a entrega de `RF46`.
...
## Critérios de aceite
- O entregável principal de `RF46` está implementado e verificável.
