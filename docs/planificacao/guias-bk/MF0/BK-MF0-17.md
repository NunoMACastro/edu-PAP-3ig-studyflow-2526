# BK-MF0-17 - Interface em português (Portugal).

## Header
- `doc_id`: `GUIA-BK-MF0-17`
- `bk_id`: `BK-MF0-17`
- `macro`: `MF0`
- `owner`: `Guilherme`
- `apoio`: `Natália`
...
Neste BK vamos entregar a capacidade correspondente a `RNF42`: **Interface em português (Portugal).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF42` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF0` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF0-18` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF0-17`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF42` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF0-18`.
...
- Quando o requisito `RNF42` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF0-17` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF42` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Interface em português (Portugal).`.
...
- [ ] Resultado observado confirma a entrega de `RNF42`.
...
## Critérios de aceite
- O entregável principal de `RNF42` está implementado e verificável.
