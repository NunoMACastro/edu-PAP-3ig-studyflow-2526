...
## Contexto do BK
- Entrega alvo: `Criar perfil IA da Área de Estudo.` com rastreabilidade direta para `RF10`.
...
- Confirmar dependencias: `BK-MF0-08`.
...
- [ ] Sei explicar como `RF10` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF0-10` e do requisito `RF10`.
2. Validar pre-condicoes técnicas e dependencias declaradas: `BK-MF0-08`.
...
export async function registarAluno(input: Credenciais) {
  ...
  return { bkId: 'BK-MF0-10', req: 'RF10', estado: 'REGISTADO' };
}
```

Garante validação mínima de identidade no arranque do fluxo de conta.
- Requisitos alvo deste BK: `RF10`.
