...
## Contexto do BK
- Entrega alvo: `Obter resumos IA baseados nos materiais enviados.` com rastreabilidade direta para `RF11`.
...
- Confirmar dependencias: `BK-MF0-08, BK-MF0-10`.
...
- [ ] Sei explicar como `RF11` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF0-11` e do requisito `RF11`.
2. Validar pre-condicoes técnicas e dependencias declaradas: `BK-MF0-08, BK-MF0-10`.
...
export async function registarAluno(input: Credenciais) {
  ...
  return { bkId: 'BK-MF0-11', req: 'RF11', estado: 'REGISTADO' };
}
```

Garante validação mínima de identidade no arranque do fluxo de conta.
- Requisitos alvo deste BK: `RF11`.
