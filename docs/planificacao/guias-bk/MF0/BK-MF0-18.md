# BK-MF0-18 - Datas no formato dd/mm/aaaa.

## Header
- `doc_id`: `GUIA-BK-MF0-18`
- `bk_id`: `BK-MF0-18`
- `macro`: `MF0`
- `owner`: `Guilherme`
- `apoio`: `Natália`
...
Neste BK vamos entregar a capacidade correspondente a `RNF43`: **Datas no formato dd/mm/aaaa.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF43` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF0` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF0-19` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF0-18`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF43` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF0-19`.
...
- Quando o requisito `RNF43` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF0-18` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF43` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Datas no formato dd/mm/aaaa.`.
...
- [ ] Resultado observado confirma a entrega de `RNF43`.
...
## Critérios de aceite
- O entregável principal de `RNF43` está implementado e verificável.
