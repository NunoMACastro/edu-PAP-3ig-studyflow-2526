# BK-MF0-11 - Regras básicas de acessibilidade (contraste, labels).
...
Neste BK vamos entregar a capacidade correspondente a `RNF05`: **Regras básicas de acessibilidade (contraste, labels).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF05` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF0` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF0-12` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF0-11`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF05` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF0-12`.
...
- Quando o requisito `RNF05` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF0-11` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF05` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Regras básicas de acessibilidade (contraste, labels).`.
...
- [ ] Resultado observado confirma a entrega de `RNF05`.
...
## Critérios de aceite
- O entregável principal de `RNF05` está implementado e verificável.
