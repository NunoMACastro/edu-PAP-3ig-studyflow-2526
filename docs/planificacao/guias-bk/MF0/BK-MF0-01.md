...
Neste BK vamos entregar a capacidade correspondente a `RF01`: **Registo do aluno (email/password ou SSO escolar).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF01` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF0` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF0-02` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF0-01`.

## O que entra (scope)
- Definir entrega mínima validável para `RF01` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF0-02`.
...
- Quando o requisito `RF01` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF0-01` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF01` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Registo do aluno (email/password ou SSO escolar).`.
...
- [ ] Resultado observado confirma a entrega de `RF01`.
...
## Critérios de aceite
- O entregável principal de `RF01` está implementado e verificável.
