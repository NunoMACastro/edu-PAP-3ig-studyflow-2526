...
Neste BK vamos entregar a capacidade correspondente a `RF05`: **O aluno pode criar rotinas e objetivos de estudo.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF0` (Fundamentos e governance) e na `Fase 1`. Dependências atuais: BK-MF0-03; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF05` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF0` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF0-06` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF0-05`.

## O que entra (scope)
- Definir entrega mínima validável para `RF05` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF0-06`.
...
- Quando o requisito `RF05` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF0-05` e dependências `BK-MF0-03`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF05` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `O aluno pode criar rotinas e objetivos de estudo.`.
...
- [ ] Resultado observado confirma a entrega de `RF05`.
...
## Critérios de aceite
- O entregável principal de `RF05` está implementado e verificável.
