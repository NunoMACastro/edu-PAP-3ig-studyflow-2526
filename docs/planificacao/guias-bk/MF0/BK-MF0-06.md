...
Neste BK vamos entregar a capacidade correspondente a `RF06`: **O aluno pode consultar histórico de estudo.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF06` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF0` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF0-07` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF0-06`.

## O que entra (scope)
- Definir entrega mínima validável para `RF06` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF0-07`.
...
- Quando o requisito `RF06` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF0-06` e dependências `BK-MF0-03`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF06` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `O aluno pode consultar histórico de estudo.`.
...
- [ ] Resultado observado confirma a entrega de `RF06`.
...
## Critérios de aceite
- O entregável principal de `RF06` está implementado e verificável.
