...
Neste BK vamos entregar a capacidade correspondente a `RF03`: **Perfil editável (nome, ano, curso, turma).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF0` (Fundamentos e governance) e na `Fase 1`. Dependências atuais: BK-MF0-02; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF03` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF0` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF0-04` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF0-03`.

## O que entra (scope)
- Definir entrega mínima validável para `RF03` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF0-04`.
...
- Quando o requisito `RF03` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF0-03` e dependências `BK-MF0-02`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF03` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Perfil editável (nome, ano, curso, turma).`.
...
- [ ] Resultado observado confirma a entrega de `RF03`.
...
## Critérios de aceite
- O entregável principal de `RF03` está implementado e verificável.
