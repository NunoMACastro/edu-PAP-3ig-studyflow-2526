# BK-MF0-14 - Backend modular por domínios (aluno, professor, IA, materiais).

## Header
- `doc_id`: `GUIA-BK-MF0-14`
- `bk_id`: `BK-MF0-14`
- `macro`: `MF0`
- `owner`: `Guilherme`
- `apoio`: `Natália`
...
Neste BK vamos entregar a capacidade correspondente a `RNF25`: **Backend modular por domínios (aluno, professor, IA, materiais).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF0` (Fundamentos e governance) e na `Fase 1`. Dependências atuais: -; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RNF25` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF0` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF0-15` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF0-14`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF25` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF0-15`.
...
- Quando o requisito `RNF25` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF0-14` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF25` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Backend modular por domínios (aluno, professor, IA, materiais).`.
...
- [ ] Resultado observado confirma a entrega de `RNF25`.
...
## Critérios de aceite
- O entregável principal de `RNF25` está implementado e verificável.
