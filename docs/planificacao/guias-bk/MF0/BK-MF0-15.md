# BK-MF0-15 - Frontend componentizado e reutilizável.

## Header
- `doc_id`: `GUIA-BK-MF0-15`
- `bk_id`: `BK-MF0-15`
- `macro`: `MF0`
- `owner`: `Guilherme`
- `apoio`: `Natália`
...
Neste BK vamos entregar a capacidade correspondente a `RNF26`: **Frontend componentizado e reutilizável.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF26` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF0` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF0-16` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF0-15`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF26` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF0-16`.
...
- Quando o requisito `RNF26` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF0-15` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF26` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Frontend componentizado e reutilizável.`.
...
- [ ] Resultado observado confirma a entrega de `RNF26`.
...
## Critérios de aceite
- O entregável principal de `RNF26` está implementado e verificável.
