# BK-MF8-01 - Integração com calendários (ICS/Google).
...
Neste BK vamos entregar a capacidade correspondente a `RF60`: **Integração com calendários (ICS/Google).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF60` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF8` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF8-02` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF8-01`.

## O que entra (scope)
- Definir entrega mínima validável para `RF60` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF8-02`.
...
- Quando o requisito `RF60` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF8-01` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF60` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Integração com calendários (ICS/Google).`.
...
- [ ] Resultado observado confirma a entrega de `RF60`.
...
## Critérios de aceite
- O entregável principal de `RF60` está implementado e verificável.
