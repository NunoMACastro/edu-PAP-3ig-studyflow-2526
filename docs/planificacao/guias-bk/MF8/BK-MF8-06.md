# BK-MF8-06 - Suportar ≥ 200 utilizadores simultâneos por escola.
...
Neste BK vamos entregar a capacidade correspondente a `RNF10`: **Suportar ≥ 200 utilizadores simultâneos por escola.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF10` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF8` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF8-07` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF8-06`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF10` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF8-07`.
...
- Quando o requisito `RNF10` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF8-06` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF10` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Suportar ≥ 200 utilizadores simultâneos por escola.`.
...
- [ ] Resultado observado confirma a entrega de `RNF10`.
...
## Critérios de aceite
- O entregável principal de `RNF10` está implementado e verificável.
