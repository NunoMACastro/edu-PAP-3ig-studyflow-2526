# BK-MF8-03 - Single Sign-On escolar (OAuth/SAML).
...
Neste BK vamos entregar a capacidade correspondente a `RF62`: **Single Sign-On escolar (OAuth/SAML).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF62` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF8` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF8-04` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF8-03`.

## O que entra (scope)
- Definir entrega mínima validável para `RF62` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF8-04`.
...
- Quando o requisito `RF62` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF8-03` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF62` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Single Sign-On escolar (OAuth/SAML).`.
...
- [ ] Resultado observado confirma a entrega de `RF62`.
...
## Critérios de aceite
- O entregável principal de `RF62` está implementado e verificável.
