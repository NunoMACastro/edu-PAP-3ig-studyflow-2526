# BK-MF8-09 - Suporte a importação UTF-8 e PT-PT.
...
Neste BK vamos entregar a capacidade correspondente a `RNF39`: **Suporte a importação UTF-8 e PT-PT.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF39` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF8` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF8-10` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF8-09`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF39` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF8-10`.
...
- Quando o requisito `RNF39` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF8-09` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF39` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Suporte a importação UTF-8 e PT-PT.`.
...
- [ ] Resultado observado confirma a entrega de `RNF39`.
...
## Critérios de aceite
- O entregável principal de `RNF39` está implementado e verificável.
