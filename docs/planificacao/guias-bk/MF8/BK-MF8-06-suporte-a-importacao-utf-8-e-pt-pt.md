...
- [ ] Sei apontar o requisito `RNF39` e demostrar cobertura objetiva.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF8-06: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bkId: 'BK-MF8-06', payload };
}
```
