# BK-MF8-08 - Compatível com Chrome, Edge, Firefox, Safari.
...
Neste BK vamos entregar a capacidade correspondente a `RNF38`: **Compatível com Chrome, Edge, Firefox, Safari.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF38` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF8` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF8-09` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF8-08`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF38` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF8-09`.
...
- Quando o requisito `RNF38` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF8-08` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF38` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Compatível com Chrome, Edge, Firefox, Safari.`.
...
- [ ] Resultado observado confirma a entrega de `RNF38`.
...
## Critérios de aceite
- O entregável principal de `RNF38` está implementado e verificável.
