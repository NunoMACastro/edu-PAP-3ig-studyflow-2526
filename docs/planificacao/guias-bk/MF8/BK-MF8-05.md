# BK-MF8-05 - Respostas da IA devem surgir em ≤ 4s.
...
Neste BK vamos entregar a capacidade correspondente a `RNF09`: **Respostas da IA devem surgir em ≤ 4s.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF09` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF8` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF8-06` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF8-05`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF09` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF8-06`.
...
- Quando o requisito `RNF09` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF8-05` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF09` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Respostas da IA devem surgir em ≤ 4s.`.
...
- [ ] Resultado observado confirma a entrega de `RNF09`.
...
## Critérios de aceite
- O entregável principal de `RNF09` está implementado e verificável.
