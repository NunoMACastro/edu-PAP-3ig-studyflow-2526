...
## Contexto do BK
- Entrega alvo: `Extrair tópicos, secções, estrutura e referências.` com rastreabilidade direta para `RF32`.
...
- [ ] Sei explicar como `RF32` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF2-08` e do requisito `RF32`.
...
export function enfileirarIndexacao(material: Material) {
  ...
  return { bkId: 'BK-MF2-08', req: 'RF32', job: `IDX-${material.id}`, estado: 'PENDING' };
}
```

Separa validação e indexação para não bloquear o utilizador.
- Requisitos alvo deste BK: `RF32`.
