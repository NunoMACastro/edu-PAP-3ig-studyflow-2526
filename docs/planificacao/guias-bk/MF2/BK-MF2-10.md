# BK-MF2-10 - IA deve ajustar explicações ao perfil do aluno.
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF1-07`
- `rf_rnf`: `RF40`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar a capacidade correspondente a `RF40`: **IA deve ajustar explicações ao perfil do aluno.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF2` (Conhecimento + IA core) e na `Fase 1`. Dependências atuais: BK-MF1-07; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF40` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF2` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF2-11` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF2-10`.

## O que entra (scope)
- Definir entrega mínima validável para `RF40` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF2-11`.
...
- Quando o requisito `RF40` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF2-10` e dependências `BK-MF1-07`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF40` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `IA deve ajustar explicações ao perfil do aluno.`.
...
- [ ] Resultado observado confirma a entrega de `RF40`.
...
## Critérios de aceite
- O entregável principal de `RF40` está implementado e verificável.
