# BK-MF2-08 - IA não pode inventar conteúdo (citações obrigatórias).
...
Neste BK vamos entregar a capacidade correspondente a `RF38`: **IA não pode inventar conteúdo (citações obrigatórias).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF2` (Conhecimento + IA core) e na `Fase 1`. Dependências atuais: BK-MF2-01; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF38` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF2` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF2-09` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF2-08`.

## O que entra (scope)
- Definir entrega mínima validável para `RF38` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF2-09`.
...
- Quando o requisito `RF38` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF2-08` e dependências `BK-MF2-01`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF38` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `IA não pode inventar conteúdo (citações obrigatórias).`.
...
- [ ] Resultado observado confirma a entrega de `RF38`.
...
## Critérios de aceite
- O entregável principal de `RF38` está implementado e verificável.
