# BK-MF2-02 - Extrair tópicos, secções, estrutura e referências.
...
Neste BK vamos entregar a capacidade correspondente a `RF32`: **Extrair tópicos, secções, estrutura e referências.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF32` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF2` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF2-03` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF2-02`.

## O que entra (scope)
- Definir entrega mínima validável para `RF32` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF2-03`.
...
- Quando o requisito `RF32` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF2-02` e dependências `BK-MF2-01`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF32` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Extrair tópicos, secções, estrutura e referências.`.
...
- [ ] Resultado observado confirma a entrega de `RF32`.
...
## Critérios de aceite
- O entregável principal de `RF32` está implementado e verificável.
