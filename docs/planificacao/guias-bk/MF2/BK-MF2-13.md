# BK-MF2-13 - Guardrails obrigatórios na IA.

## Header
- `doc_id`: `GUIA-BK-MF2-13`
- `bk_id`: `BK-MF2-13`
- `macro`: `MF2`
- `owner`: `Natália`
- `apoio`: `Guilherme`
...
Neste BK vamos entregar a capacidade correspondente a `RNF19`: **Guardrails obrigatórios na IA.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF19` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF2` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF2-14` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF2-13`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF19` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF2-14`.
...
- Quando o requisito `RNF19` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF2-13` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF19` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Guardrails obrigatórios na IA.`.
...
- [ ] Resultado observado confirma a entrega de `RNF19`.
...
## Critérios de aceite
- O entregável principal de `RNF19` está implementado e verificável.
