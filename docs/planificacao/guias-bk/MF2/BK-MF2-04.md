# BK-MF2-04 - Separar materiais entre “aluno”, “professor” e “turma”.
...
Neste BK vamos entregar a capacidade correspondente a `RF34`: **Separar materiais entre “aluno”, “professor” e “turma”.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF34` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF2` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF2-05` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF2-04`.

## O que entra (scope)
- Definir entrega mínima validável para `RF34` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF2-05`.
...
- Quando o requisito `RF34` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF2-04` e dependências `BK-MF2-01`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF34` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Separar materiais entre “aluno”, “professor” e “turma”.`.
...
- [ ] Resultado observado confirma a entrega de `RF34`.
...
## Critérios de aceite
- O entregável principal de `RF34` está implementado e verificável.
