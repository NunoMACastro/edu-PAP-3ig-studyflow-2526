# BK-MF2-06 - Assistente IA da disciplina/turma com voz docente.
...
Neste BK vamos entregar a capacidade correspondente a `RF36`: **Assistente IA da disciplina/turma com voz docente.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF2` (Conhecimento + IA core) e na `Fase 1`. Dependências atuais: BK-MF3-04; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF36` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF2` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF2-07` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF2-06`.

## O que entra (scope)
- Definir entrega mínima validável para `RF36` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF2-07`.
...
- Quando o requisito `RF36` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF2-06` e dependências `BK-MF3-04`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF36` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Assistente IA da disciplina/turma com voz docente.`.
...
- [ ] Resultado observado confirma a entrega de `RF36`.
...
## Critérios de aceite
- O entregável principal de `RF36` está implementado e verificável.
