# BK-MF2-01 - Indexação automática de PDFs, DOCX e URLs.
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF1-02, BK-MF3-03`
- `rf_rnf`: `RF31`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar a capacidade correspondente a `RF31`: **Indexação automática de PDFs, DOCX e URLs.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF2` (Conhecimento + IA core) e na `Fase 1`. Dependências atuais: BK-MF1-02, BK-MF3-03; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF31` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF2` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF2-02` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF2-01`.

## O que entra (scope)
- Definir entrega mínima validável para `RF31` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF2-02`.
...
- Quando o requisito `RF31` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF2-01` e dependências `BK-MF1-02, BK-MF3-03`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF31` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Indexação automática de PDFs, DOCX e URLs.`.
...
- [ ] Resultado observado confirma a entrega de `RF31`.
...
## Critérios de aceite
- O entregável principal de `RF31` está implementado e verificável.
