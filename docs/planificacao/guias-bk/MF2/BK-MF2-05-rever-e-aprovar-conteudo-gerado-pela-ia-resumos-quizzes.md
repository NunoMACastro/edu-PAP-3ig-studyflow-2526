...
## Contexto do BK
- Entrega alvo: `Rever e aprovar conteúdo gerado pela IA (resumos/quizzes).` com rastreabilidade direta para `RF29`.
...
- [ ] Sei explicar como `RF29` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF2-05` e do requisito `RF29`.
...
export function calcularDesempenho(respostas: Resposta[]) {
  ...
  return { bkId: 'BK-MF2-05', req: 'RF29', score: Math.round((corretas / respostas.length) * 100) };
}
```

Produz saída objetiva por tópico para acompanhamento docente.
- Requisitos alvo deste BK: `RF29`.
