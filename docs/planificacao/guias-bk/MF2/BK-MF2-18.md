# BK-MF2-18 - IA não pode inventar informação factual.

## Header
- `doc_id`: `GUIA-BK-MF2-18`
- `bk_id`: `BK-MF2-18`
- `macro`: `MF2`
- `owner`: `Natália`
- `apoio`: `Guilherme`
...
Neste BK vamos entregar a capacidade correspondente a `RNF35`: **IA não pode inventar informação factual.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF35` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF2` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF2-19` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF2-18`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF35` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF2-19`.
...
- Quando o requisito `RNF35` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF2-18` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF35` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `IA não pode inventar informação factual.`.
...
- [ ] Resultado observado confirma a entrega de `RNF35`.
...
## Critérios de aceite
- O entregável principal de `RNF35` está implementado e verificável.
