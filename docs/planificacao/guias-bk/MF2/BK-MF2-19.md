# BK-MF2-19 - IA externa segue políticas e filtros próprios.

## Header
- `doc_id`: `GUIA-BK-MF2-19`
- `bk_id`: `BK-MF2-19`
- `macro`: `MF2`
- `owner`: `Natália`
- `apoio`: `Guilherme`
...
Neste BK vamos entregar a capacidade correspondente a `RNF37`: **IA externa segue políticas e filtros próprios.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF37` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF2` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF3-01` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF2-19`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF37` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF3-01`.
...
- Quando o requisito `RNF37` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF2-19` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF37` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `IA externa segue políticas e filtros próprios.`.
...
- [ ] Resultado observado confirma a entrega de `RNF37`.
...
## Critérios de aceite
- O entregável principal de `RNF37` está implementado e verificável.
