# BK-MF2-11 - Indexação de documentos deve ser assíncrona e não bloquear UI.
...
Neste BK vamos entregar a capacidade correspondente a `RNF11`: **Indexação de documentos deve ser assíncrona e não bloquear UI.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF11` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF2` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF2-12` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF2-11`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF11` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF2-12`.
...
- Quando o requisito `RNF11` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF2-11` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF11` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Indexação de documentos deve ser assíncrona e não bloquear UI.`.
...
- [ ] Resultado observado confirma a entrega de `RNF11`.
...
## Critérios de aceite
- O entregável principal de `RNF11` está implementado e verificável.
