# BK-MF2-16 - IA respeita perfis distintos (aluno, turma, professor).

## Header
- `doc_id`: `GUIA-BK-MF2-16`
- `bk_id`: `BK-MF2-16`
- `macro`: `MF2`
- `owner`: `Natália`
- `apoio`: `Guilherme`
...
Neste BK vamos entregar a capacidade correspondente a `RNF32`: **IA respeita perfis distintos (aluno, turma, professor).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF32` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF2` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF2-17` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF2-16`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF32` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF2-17`.
...
- Quando o requisito `RNF32` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF2-16` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF32` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `IA respeita perfis distintos (aluno, turma, professor).`.
...
- [ ] Resultado observado confirma a entrega de `RNF32`.
...
## Critérios de aceite
- O entregável principal de `RNF32` está implementado e verificável.
