# BK-MF2-14 - IA não acede a dados de outras turmas ou alunos.

## Header
- `doc_id`: `GUIA-BK-MF2-14`
- `bk_id`: `BK-MF2-14`
- `macro`: `MF2`
- `owner`: `Natália`
- `apoio`: `Guilherme`
...
Neste BK vamos entregar a capacidade correspondente a `RNF20`: **IA não acede a dados de outras turmas ou alunos.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RNF20` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF2` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF2-15` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF2-14`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF20` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF2-15`.
...
- Quando o requisito `RNF20` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF2-14` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF20` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `IA não acede a dados de outras turmas ou alunos.`.
...
- [ ] Resultado observado confirma a entrega de `RNF20`.
...
## Critérios de aceite
- O entregável principal de `RNF20` está implementado e verificável.
