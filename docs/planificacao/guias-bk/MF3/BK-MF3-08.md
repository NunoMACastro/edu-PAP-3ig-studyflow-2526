# BK-MF3-08 - IA segue limites definidos pelo professor.
...
Neste BK vamos entregar a capacidade correspondente a `RNF33`: **IA segue limites definidos pelo professor.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF3` (Professor/turmas) e na `Fase 2`. Dependências atuais: -; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RNF33` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF3` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF4-01` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF3-08`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF33` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF4-01`.
...
- Quando o requisito `RNF33` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF3-08` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF33` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `IA segue limites definidos pelo professor.`.
...
- [ ] Resultado observado confirma a entrega de `RNF33`.
...
## Critérios de aceite
- O entregável principal de `RNF33` está implementado e verificável.
