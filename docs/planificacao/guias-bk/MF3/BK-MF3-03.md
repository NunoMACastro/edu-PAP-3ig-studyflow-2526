# BK-MF3-03 - Submeter materiais da disciplina (versão oficial).
...
Neste BK vamos entregar a capacidade correspondente a `RF21`: **Submeter materiais da disciplina (versão oficial).**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF3` (Professor/turmas) e na `Fase 2`. Dependências atuais: BK-MF3-02; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF21` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF3` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF3-04` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF3-03`.

## O que entra (scope)
- Definir entrega mínima validável para `RF21` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF3-04`.
...
- Quando o requisito `RF21` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF3-03` e dependências `BK-MF3-02`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF21` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Submeter materiais da disciplina (versão oficial).`.
...
- [ ] Resultado observado confirma a entrega de `RF21`.
...
## Critérios de aceite
- O entregável principal de `RF21` está implementado e verificável.
