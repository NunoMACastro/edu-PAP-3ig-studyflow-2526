...
## Contexto do BK
- Entrega alvo: `Pesquisa unificada por tópico/conceito/material.` com rastreabilidade direta para `RF45`.
...
- [ ] Sei explicar como `RF45` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF3-09` e do requisito `RF45`.
...
export async function registarAluno(input: Credenciais) {
  ...
  return { bkId: 'BK-MF3-09', req: 'RF45', estado: 'REGISTADO' };
}
```

Garante validação mínima de identidade no arranque do fluxo de conta.
- Requisitos alvo deste BK: `RF45`.
