...
## Contexto do BK
- Entrega alvo: `Configurar preferências de notificações (email, push, app) por contexto.` com rastreabilidade direta para `RF47`.
...
- [ ] Sei explicar como `RF47` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF3-11` e do requisito `RF47`.
...
export function podeNotificar(pref: Preferencia, enviadosHoje: number, quota: number) {
  if (!pref.ativo) return { bkId: 'BK-MF3-11', req: 'RF47', enviar: false, motivo: 'opt-out' };
  return { bkId: 'BK-MF3-11', req: 'RF47', enviar: enviadosHoje < quota };
}
```

Impõe preferências e quota máxima antes do envio.
- Requisitos alvo deste BK: `RF47`.
