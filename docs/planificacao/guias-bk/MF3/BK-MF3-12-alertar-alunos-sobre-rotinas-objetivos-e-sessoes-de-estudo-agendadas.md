...
## Contexto do BK
- Entrega alvo: `Alertar alunos sobre rotinas, objetivos e sessões de estudo agendadas.` com rastreabilidade direta para `RF48`.
...
- Confirmar dependencias: `BK-MF0-05`.
...
- [ ] Sei explicar como `RF48` se traduz em comportamento implementável.
...
### Passos
1. Confirmar no backlog e na matriz o escopo de `BK-MF3-12` e do requisito `RF48`.
2. Validar pre-condicoes técnicas e dependencias declaradas: `BK-MF0-05`.
...
export function podeNotificar(pref: Preferencia, enviadosHoje: number, quota: number) {
  if (!pref.ativo) return { bkId: 'BK-MF3-12', req: 'RF48', enviar: false, motivo: 'opt-out' };
  return { bkId: 'BK-MF3-12', req: 'RF48', enviar: enviadosHoje < quota };
}
```

Impõe preferências e quota máxima antes do envio.
- Requisitos alvo deste BK: `RF48`.
