# BK-MF3-04 - Configurar “voz da IA” docente.
...
Neste BK vamos entregar a capacidade correspondente a `RF22`: **Configurar “voz da IA” docente.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF3` (Professor/turmas) e na `Fase 2`. Dependências atuais: BK-MF3-03; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF22` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF3` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF3-05` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF3-04`.

## O que entra (scope)
- Definir entrega mínima validável para `RF22` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF3-05`.
...
- Quando o requisito `RF22` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF3-04` e dependências `BK-MF3-03`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF22` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Configurar “voz da IA” docente.`.
...
- [ ] Resultado observado confirma a entrega de `RF22`.
...
## Critérios de aceite
- O entregável principal de `RF22` está implementado e verificável.
