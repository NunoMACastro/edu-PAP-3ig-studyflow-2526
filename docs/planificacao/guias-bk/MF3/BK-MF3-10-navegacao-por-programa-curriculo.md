...
- [ ] Sei apontar o requisito `RF46` e demostrar cobertura objetiva.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF3-10: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bkId: 'BK-MF3-10', payload };
}
```
