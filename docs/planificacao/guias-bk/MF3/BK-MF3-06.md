# BK-MF3-06 - Professores podem enviar avisos e publicações.
...
Neste BK vamos entregar a capacidade correspondente a `RF24`: **Professores podem enviar avisos e publicações.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF24` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF3` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF3-07` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF3-06`.

## O que entra (scope)
- Definir entrega mínima validável para `RF24` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF3-07`.
...
- Quando o requisito `RF24` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF3-06` e dependências `BK-MF3-01`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF24` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Professores podem enviar avisos e publicações.`.
...
- [ ] Resultado observado confirma a entrega de `RF24`.
...
## Critérios de aceite
- O entregável principal de `RF24` está implementado e verificável.
