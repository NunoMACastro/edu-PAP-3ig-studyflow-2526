# BK-MF3-05 - O aluno inscrito numa turma recebe versão limitada da IA.
...
- `estado`: `TODO`
- `esforço`: `M`
- `dependências`: `BK-MF3-04`
- `rf_rnf`: `RF23`
- `last_updated`: `2026-04-12`

## O que vamos fazer neste BK
Neste BK vamos entregar a capacidade correspondente a `RF23`: **O aluno inscrito numa turma recebe versão limitada da IA.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF3` (Professor/turmas) e na `Fase 2`. Dependências atuais: BK-MF3-04; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF23` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF3` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF3-06` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF3-05`.

## O que entra (scope)
- Definir entrega mínima validável para `RF23` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF3-06`.
...
- Quando o requisito `RF23` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF3-05` e dependências `BK-MF3-04`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF23` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `O aluno inscrito numa turma recebe versão limitada da IA.`.
...
- [ ] Resultado observado confirma a entrega de `RF23`.
...
## Critérios de aceite
- O entregável principal de `RF23` está implementado e verificável.
