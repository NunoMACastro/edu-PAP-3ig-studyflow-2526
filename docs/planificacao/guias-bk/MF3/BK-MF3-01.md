...
Neste BK vamos entregar a capacidade correspondente a `RF19`: **Criar turmas.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.
...
- Impacto funcional no produto: concretiza `RF19` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF3` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF3-02` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF3-01`.

## O que entra (scope)
- Definir entrega mínima validável para `RF19` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF3-02`.
...
- Quando o requisito `RF19` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF3-01` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF19` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Criar turmas.`.
...
- [ ] Resultado observado confirma a entrega de `RF19`.
...
## Critérios de aceite
- O entregável principal de `RF19` está implementado e verificável.
