# BK-MF4-05 - Painel com progresso, dificuldades e métricas da turma.
...
Neste BK vamos entregar a capacidade correspondente a `RF30`: **Painel com progresso, dificuldades e métricas da turma.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF4` (Projetos/testes/curadoria) e na `Fase 2`. Dependências atuais: BK-MF3-06; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF30` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF4` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF4-06` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF4-05`.

## O que entra (scope)
- Definir entrega mínima validável para `RF30` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF4-06`.
...
- Quando o requisito `RF30` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF4-05` e dependências `BK-MF3-06`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF30` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Painel com progresso, dificuldades e métricas da turma.`.
...
- [ ] Resultado observado confirma a entrega de `RF30`.
...
## Critérios de aceite
- O entregável principal de `RF30` está implementado e verificável.
