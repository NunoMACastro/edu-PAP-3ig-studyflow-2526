...
- [ ] Sei apontar o requisito `RF53` e demostrar cobertura objetiva.
...
export function validarEntradaBK(payload: Payload) {
  ...
  if (emFalta.length) throw new Error(`BK BK-MF4-05: faltam campos ${emFalta.join(', ')}`);
  return { ok: true, bkId: 'BK-MF4-05', payload };
}
```
