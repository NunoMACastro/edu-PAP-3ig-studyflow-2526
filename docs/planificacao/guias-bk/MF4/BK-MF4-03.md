# BK-MF4-03 - Criar testes/mini-testes oficiais.
...
Neste BK vamos entregar a capacidade correspondente a `RF28`: **Criar testes/mini-testes oficiais.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF4` (Projetos/testes/curadoria) e na `Fase 2`. Dependências atuais: BK-MF3-02; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF28` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF4` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF4-04` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF4-03`.

## O que entra (scope)
- Definir entrega mínima validável para `RF28` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF4-04`.
...
- Quando o requisito `RF28` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF4-03` e dependências `BK-MF3-02`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF28` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Criar testes/mini-testes oficiais.`.
...
- [ ] Resultado observado confirma a entrega de `RF28`.
...
## Critérios de aceite
- O entregável principal de `RF28` está implementado e verificável.
