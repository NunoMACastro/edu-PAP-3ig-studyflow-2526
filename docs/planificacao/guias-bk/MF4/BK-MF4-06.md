# BK-MF4-06 - Geração de quizzes em background quando necessário.
...
Neste BK vamos entregar a capacidade correspondente a `RNF12`: **Geração de quizzes em background quando necessário.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF4` (Projetos/testes/curadoria) e na `Fase 2`. Dependências atuais: -; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RNF12` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF4` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF5-01` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF4-06`.

## O que entra (scope)
- Definir entrega mínima validável para `RNF12` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF5-01`.
...
- Quando o requisito `RNF12` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF4-06` e dependências `-`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RNF12` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `Geração de quizzes em background quando necessário.`.
...
- [ ] Resultado observado confirma a entrega de `RNF12`.
...
## Critérios de aceite
- O entregável principal de `RNF12` está implementado e verificável.
