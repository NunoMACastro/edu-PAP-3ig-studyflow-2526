# BK-MF4-02 - A IA deve ajudar o aluno a elaborar projetos de forma gradual.
...
Neste BK vamos entregar a capacidade correspondente a `RF27`: **A IA deve ajudar o aluno a elaborar projetos de forma gradual.**. O resultado final observável é uma evidência objetiva de que o requisito foi implementado e validado conforme backlog.

Este trabalho decorre na macro `MF4` (Projetos/testes/curadoria) e na `Fase 2`. Dependências atuais: BK-MF4-01; se algum detalhe técnico estiver em falta, fica **a definir no BK dependente**.

## Porque isto é importante
- Impacto funcional no produto: concretiza `RF27` e reduz lacunas no fluxo do utilizador final.
- Impacto técnico/arquitetural: consolida a base técnica da macro `MF4` com critérios verificáveis.
- Impacto na sequência de BKs: desbloqueia a continuidade para `BK-MF4-03` quando aplicável.
- Risco de execução incorreta: divergência entre backlog e entrega pode quebrar rastreabilidade e aceite do `BK-MF4-02`.

## O que entra (scope)
- Definir entrega mínima validável para `RF27` alinhada com RF/RNF.
...
- Trabalho de BKs seguintes além de `BK-MF4-03`.
...
- Quando o requisito `RF27` é exercitado, então o comportamento esperado é observado sem ambiguidade.
...
1. **Baseline e dependências**
   Objetivo (~10 min): confirmar contexto `BK-MF4-02` e dependências `BK-MF4-01`.
...
2. **Decomposição de tarefas**
   Objetivo (~15 min): partir `RF27` em tarefas executáveis curtas.
...
3. **Desenho da solução**
   Objetivo (~20 min): definir abordagem técnica mínima para `A IA deve ajudar o aluno a elaborar projetos de forma gradual.`.
...
- [ ] Resultado observado confirma a entrega de `RF27`.
...
## Critérios de aceite
- O entregável principal de `RF27` está implementado e verificável.
